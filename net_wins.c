...
void WINS_GetLocalAddress()
{
	...
	addr = ntohl(myAddr);
	dpsnprintf(my_tcpip_address, sizeof(my_tcpip_address), "%d.%d.%d.%d", (addr >> 24) & 0xff, (addr >> 16) & 0xff, (addr >> 8) & 0xff, addr & 0xff);
}
...
int WINS_Read (int socket, byte *buf, int len, struct qsockaddr *addr)
{
	...
	if (ret == -1)
	{
		int errn = pWSAGetLastError();

		if (errn == WSAEWOULDBLOCK || errn == WSAECONNREFUSED)
			return 0;
